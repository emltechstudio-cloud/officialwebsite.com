...
const checks = [
  ...
  ['profile.html', ['eml_youniversity_learner', 'eml_youniversity_token', 'dashboard.html', 'learner_id']],
  ['assessment.html', ['/final-exam', 'eml_youniversity_token', 'portal.html?course=', 'dashboard.html']],
];
...
const worker = fs.readFileSync('/home/ubuntu/youniversity-html-review/university-service-worker.js', 'utf8');
 for (const value of ['University Service Worker', 'dashboard.html', 'profile.html', 'assessment.html', 'offline.html', 'updates.json', 'EML_YOUNIVERSITY_ACTIVATE_UPDATE']) if (!worker.includes(value)) throw new Error(`University Service Worker is missing ${value}`);
